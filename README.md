...
- Adjust image properties including contrast, saturation, shadows, and highlights.
- Control LUT intensity from 0–100% for customizable film blending.
- Live shader-based preview while editing images.
- Film grain generation support.
...
- **Skia Runtime Shaders**: Provides live GPU-accelerated image previews during editing.
- **Android NDK / C++**: Used for native LUT processing and film grain generation on Android.
- **Core Graphics**: Used for native image processing implementation on iOS.
...
The image processing pipeline uses native implementations on both platforms along with a shared Skia shader preview pipeline for real-time editing. Tonal adjustments operate in linear light with proper sRGB encode/decode for more predictable contrast and exposure behavior.
...
5. If you modify native Android image-processing code, run:
   ```bash
   ./gradlew clean
   ```
   to rebuild the NDK libraries properly.
